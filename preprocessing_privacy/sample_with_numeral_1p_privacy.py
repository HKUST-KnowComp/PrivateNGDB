...
class GraphSamplerE34:
    ...
    def sample_with_pattern(self, pattern):
        while True:
            ...
            if _query is not None:
                return _query
    def sample_with_pattern_on_node(self, pattern, node):
        i = 0
        while True:
    ...
    def query_search_answer_with_private_attr(self, query, private_attr):
        ...
        if sub_queries[0] == "rp":
            ...
            sub_query_answers, sub_flag = self.query_search_answer_with_private_attr(sub_queries[2], private_attr)
            # print(sub_queries[0])
            # print(sub_query_answers)
            all_answers = []
            private_flag = {}
            for answer_id, sub_answer in enumerate(sub_query_answers):
                ...
                for node in next_nodes:
                    ...
                    if int(sub_queries[1][1:-1]) in graph.edges[sub_answer, node]:
                        all_answers.append(node)
                        private_flag.setdefault(node, sub_flag[sub_answer])

            all_answers = list(set(all_answers))
            return all_answers, private_flag
            ...
            for answer_id, sub_answer in enumerate(sub_query_answers):
                ...
                for node in next_nodes:
                    ...
                    if int(sub_queries[1][1:-1]) in graph.edges[sub_answer, node]:
                        all_answers.append(node)
                        if (sub_answer, node, {int(sub_queries[1][1:-1]): 1}) in private_attr:
                            private_flag.setdefault(node, True)
                        else:
                            private_flag.setdefault(node, False | sub_flag[sub_answer])
            ...
            for answer_id, sub_answer in enumerate(sub_query_answers):
                ...
                for node in next_nodes:
                    ...
                    if int(sub_queries[1][1:-1]) in graph.edges[sub_answer, node]:
                        all_answers.append(node)
                        if (node, sub_answer, {int(sub_queries[1][1:-1]): 1}) in private_attr:
                            private_flag.setdefault(node, True)
                        else:
                            private_flag.setdefault(node, False | sub_flag[sub_answer])
            all_answers = list(set(all_answers))
            ...
            return [sub_queries[1][1:-1]], {sub_queries[1][1:-1]: False}
            ...
            return [(float(numerical_value), int(type_id))], {(float(numerical_value), int(type_id)): False}
            ...
            private_flag = {}
            for i in range(1, len(sub_queries)):
                sub_query_answers_i, sub_flag_i = self.query_search_answer_with_private_attr(sub_queries[i], private_attr)
                sub_query_answers_list.append(sub_query_answers_i)
                private_flag = {key: private_flag.get(key, False) or sub_flag_i.get(key, False) for key in
                                set(private_flag) | set(sub_flag_i)}
            ...
            return merged_answers, private_flag
            ...
            for i in range(1, len(sub_queries)):
                sub_query_answers_i, sub_flag_i = self.query_search_answer_with_private_attr(sub_queries[i], private_attr)
                sub_query_answers_list.append(sub_query_answers_i)
                private_flag = {key: private_flag.get(key, True) and sub_flag_i.get(key, True) for key in
                                set(private_flag) | set(sub_flag_i)}
            ...
            sub_query_answers, sub_flag = self.query_search_answer_with_private_attr(sub_queries[1], private_attr)
            all_nodes = list(self.graph.nodes)
            negative_answers = [node for node in all_nodes if node not in sub_query_answers]

            negative_answers = list(set(negative_answers))

            return negative_answers, {node: False for node in negative_answers}
    ...
    for data_dir in n_queries_train_dict.keys():
        ...
        def sample_valid_graph_with_pattern(pattern):
            while True:
                ...
                if len(valid_query_train_answers) > 0 and 0 < len(valid_query_valid_answers) != len(
                        valid_query_train_answers):
                    break
