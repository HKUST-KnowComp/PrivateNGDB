...
dir_list = ["../sampled_data_privacy_query"]
output_dir = "../input_files_privacy/"
...
for directory_name in dir_list:
    ...
    for data_name in data_names:
        ...
        test_data_private_public_prefix = data_name  + "_test_private_public_queries"
        train_privacy_prefix = data_name  + "_train_private_queries"
        ...
        #     filehandler.close()
        #
        # print("private public test")
        # test_private_public_dict_list = []
        # for file in tqdm(all_files):
        #     if test_data_private_public_prefix in file:
        #         with open(sample_data_path + file, "r") as fin:
        #             data_dict = json.load(fin)
        #             test_private_public_dict_list.append(data_dict)
        # if len(test_private_public_dict_list) > 0:
        #     test_private_public_dict_list = merge_query_file(test_private_public_dict_list)
        #
        #     filehandler = open(output_dir + test_data_private_public_prefix + ".pkl", "wb")
        #     pickle.dump(test_private_public_dict_list, filehandler)
        #     filehandler.close()

        print("private train")
        train_private_dict_list = []
        for file in tqdm(all_files):
            if train_privacy_prefix in file:
                with open(sample_data_path + file, "r") as fin:
                    data_dict = json.load(fin)
                    train_private_dict_list.append(data_dict)
        if len(train_private_dict_list) > 0:
            train_private_dict_list = merge_query_file(train_private_dict_list)

            filehandler = open(output_dir + train_privacy_prefix + ".pkl", "wb")
            pickle.dump(train_private_dict_list, filehandler)
            filehandler.close()