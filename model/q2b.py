...
class Q2B(IterativeModel):
    ...
    # def loss_pri(self, sub_query_encoding, labels):
    #     query_scores = self.scoring(sub_query_encoding)
    #     # [batch_size]
    #
    #     n = query_scores.size(-1)
    #     log_preds = F.log_softmax(query_scores, dim=-1)
    #     _loss = -log_preds.sum(dim=-1).mean()
    #
    #     # _loss = self.label_smoothing_loss(query_scores, labels)
    #
    #     return _loss / n
    ...
    def loss_pri(self, sub_query_encoding, labels):
        query_scores = self.scoring(sub_query_encoding)
        n = query_scores.size(-1)
        preds = F.softmax(query_scores, dim=-1)
        target = torch.ones_like(query_scores).to(self.entity_embedding.weight.device)
    ...
    def forward(self, batched_structured_query, label=None, p_flag=False):
        ...
        if label is None:
            ...
            if p_flag:
                if isinstance(label[0], int) or isinstance(label[0], np.int64) or isinstance(label[0], np.int32):
                    # print("label is int")
                    # print   (label)
                    loss = self.loss_pri(this_query_result, label)
