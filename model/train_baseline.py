...
if __name__ == "__main__":
    ...
    if args.dp:
        ...
        optimizer = DPOptimizer(
            optimizer=optimizer,
            noise_multiplier=0.01,  # same as make_private arguments
        ...
        if step % save_step == 0 and not args.timing:
            ...
            for task_name, loader in baseline_validation_loaders.items():
                ...
                for batched_query, unified_ids, train_answers, valid_answers in tqdm(loader):
                    ...
                    if args.dp:
                        generalization_logs = model._module.evaluate_generalization(query_embedding, train_answers,
                                                                                    valid_answers)
                    else:
            ...
            for task_name, loader in baseline_test_loaders.items():
                ...
                for batched_query, unified_ids, train_answers, valid_answers, \
                    ...
                    query_embedding = model(batched_query)
                    if args.dp:
                        generalization_logs = model._module.evaluate_generalization(query_embedding, valid_answers,
                                                                                    test_answers)
                    else:
                    ...
                    try:
                        if args.dp:
                            generalization_logs_public = model._module.evaluate_generalization(query_embedding_sample,
                                                                                   valid_answers_sample,
