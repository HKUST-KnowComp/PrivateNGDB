...
if __name__ == "__main__":
    ...
    train_log_dir = '../logs/gradient_tape/' + current_time + "_" + args.model + "_" + data_name + '_baseline_privacy_new'
    test_log_dir = '../logs/gradient_tape/' + current_time + "_" + args.model + "_" + data_name + '_baseline_privacy_new'

    if args.mi:
        train_log_dir += '_mi_{}'.format(args.privacy_multiplier)
        test_log_dir += '_mi_{}'.format(args.privacy_multiplier)

    if args.cl:
        train_log_dir += '_cl_{}'.format(args.privacy_multiplier)
        test_log_dir += '_cl_{}'.format(args.privacy_multiplier)
    ...
    optimizer_pri = torch.optim.Adam(model.entity_embedding.parameters(), lr=args.learning_rate * args.privacy_multiplier)

    def sample_negative(sample_list):
        negative_sample = []
        if sample_list[0][0] > nentity:
            value_flag = True
        else:
            value_flag = False
        for i in range(len(sample_list)):
            if value_flag:
                negative = random.choice(list(range(nentity, nentity + nvalue)))
            else:
                negative = random.choice(list(range(nentity)))
    ...
    for step in tqdm(range(args.max_train_step)):
        ...
        batched_query, unified_ids, positive_sample, _ = next(iterator)
        ...
        step_flag = True
        if (step) % args.num_private_steps == 0:
            if args.mi:
                private_task = np.random.choice(list(extended_private_query_iterators.keys()))
                iterator = extended_private_query_iterators[private_task]
                batched_query, unified_ids, positive_sample, _ = next(iterator)
                loss_pri = model(batched_query, positive_sample)
                loss = loss - args.privacy_multiplier * loss_pri
                # loss_pri = model(batched_query, positive_sample, True)
                # loss = loss + args.privacy_multiplier * loss_pri

            elif args.cl:
                private_task = np.random.choice(list(extended_private_train_query_iterators.keys()))
                iterator = extended_private_train_query_iterators[private_task]
                batched_query, unified_ids, positive_sample, sample_list = next(iterator)
                # loss_pri = model(batched_query, sample_list, True)

                # negative_sample = sample_negative(sample_list)
                # loss_pri = model(batched_query, negative_sample)
        ...
        if step % 100 == 0:
            ...
            if args.cl:
                train_summary_writer.add_scalar("y-train-private-" + private_task, loss_pri.item(), step)
        ...
        if step % save_step == 0 and not args.timing:
            ...
            for task_name, loader in baseline_validation_loaders.items():
                ...
                for batched_query, unified_ids, train_answers, valid_answers in tqdm(loader):
                    ...
                    generalization_logs = model.evaluate_generalization(query_embedding,
                                                                        train_answers, valid_answers)
                ...
                for batched_query, unified_ids, train_answers, valid_answers, \
                    ...
                    if True:
                        ...
                        if abstract_query_type in abstracted_implicit_generalization_logs:
                            ...
                            if flag_private:
                                try:
                                    abstracted_implicit_generalization_logs_private[abstract_query_type].extend(generalization_logs_private)
                                except:
                            ...
                            if flag_public:
                                try:
                                    abstracted_implicit_generalization_logs_public[abstract_query_type].extend(generalization_logs_public)
                                except:
